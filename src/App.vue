...
<template>
  <div>
    <div class="card">
      ...
      <DataTable id="datatable" ref="dt" :value="players" v-model:selection="selectedPlayers" dataKey="id"
                 :paginator="true" :rows="10" :filters="filters"
                 paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                 :rowsPerPageOptions="[5,10,25]"
                 currentPageReportTemplate="Showing {first} to {last} of {totalRecords} players">
    ...
    <Dialog v-model:visible="playerDialog" :style="{width: '450px'}" header="Player Details" :modal="true"
      ...
      <div class="field">
        ...
        <InputText
            id="playerRole"
            v-bind="playerRole"
            v-model.trim="player.playerRole"
            aria-describedby="player-role-help"
            :class="{ 'p-invalid': errors.playerRole }"
        />
        <small id="player-role-help" class="p-error">
          {{ errors.playerRole }}
        </small>
      </div>
      <template #footer>
...
const schema = yup.object({
  ...
  playerRole: yup.string().required().label('Player Role'),
});
...
const playerRole = defineComponentBinds('playerRole');
const resetPlayer = ref({
  id: null,
  name: "",
  address: "",
  playerRole: ""
...
const submitted = ref(false);
const openNew = () => {
  player.value = resetPlayer.value;
  submitted.value = false;
  playerDialog.value = true;
};
const hideDialog = () => {
